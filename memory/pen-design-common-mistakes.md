...
- **对**：优先用组件库 ref（如 `devui:Rpv7B` 顶栏、`devui:JGEqF` 侧栏、`devui:35iCd` 表格），用 `descendants` 或 `U(实例ID/devui:子节点ID, {content: "..."})` 改文案。
...
- **错**：用 35iCd 后不改表头，仍显示「编号」「标题」「结束时间」「状态」等。
- **对**：查表头 text 节点 ID（如 ZNuLN、ch95U…），用 `U(表格实例ID/devui:xxx, {content: "代码仓"})` 等改成业务列名；不需要的列可 `width:0` 隐藏。
